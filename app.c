...
zaf_event_distributor_app_event_manager(uint8_t event)
{
  ...
  switch (event)
  {
    case EVENT_APP_TOGGLE_LED1:
      ...
      break;
    case EVENT_APP_TOGGLE_LED2:

          p_switches = cc_binary_switch_get_config();
          if (cc_binary_switch_get_current_value(&p_switches[1]) > 0)
            {
              cc_binary_switch_set(&p_switches[1], 0);
            }
          else
            {
              cc_binary_switch_set(&p_switches[1], 0xFF); // 0xFF for last on value.
            }
          break;
    case EVENT_APP_TOGGLE_LED3:

          p_switches = cc_binary_switch_get_config();
          if (cc_binary_switch_get_current_value(&p_switches[2]) > 0)
            {
              cc_binary_switch_set(&p_switches[2], 0);
            }
          else
            {
              cc_binary_switch_set(&p_switches[2], 0xFF); // 0xFF for last on value.
            }
          break;
